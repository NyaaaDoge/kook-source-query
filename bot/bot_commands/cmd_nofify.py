...
def reg_notify_cmd(bot: Bot):
    ...
    async def notify(msg: Message, command: str = None, *args):
        ...
        if not command:
            await msg.reply("用法：\n`/notify map [完整地图名]` - 订阅特定地图。当管理员设置的监控服务器里面有你订阅的地图名时，Bot将私信通知您。\n"
            ...
            if not any(args):
                await msg.reply("用法：`/notify map [map_name]` - 订阅特定地图，每当监测到该地图时会进行私信推送通知。\n"
                ...
                map_name = re.sub(r'[^a-zA-Z_\d]', '', input_str)
            ...
            if not any(args):
                await msg.reply("用法：`/notify unsub [map_name]` - 取消订阅特定地图，每当监测到该地图进行推送通知。"
                                "请使用 [完整地图名] 如 ze_2012_p3, ze_k19_escape_go1 等格式来订阅，需要版本号精确匹配。\n"
            ...
            if not any(args):
                ...
                if delete_flag:
                    await msg.reply(f":green_square: 地图订阅列表删除成功，总计删除了 {delete_flag} 个订阅。")
                else:
