...
def query_server_result_card_msg(ip_addr, server_info: SourceInfo,
    ...
    if show_ip:
        card.append(Module.Section(Element.Text(f"{ip_addr}")))
    if map_img:
...
def query_server_results_batch_card_msg(server_info_list: list,
    ...
    for server_info in server_info_list[:15]:
        ...
        if map_img:
            ...
            search_result = search_map(server_info.map_name, map_list)
            if search_result:
                card.append(Module.Container(Element.Image(src=search_result['medium'], circle=False, size=Types.Size.LG)))
    card_msg.append(card)
    return card_msg
